...
/**
 * Tools that computes all the maximal symbolic steady states in a given
 * regulatory graph.
 *
...
public class Searcher implements Runnable {
    ...
    private RegulatoryGraph regulatoryGraph;
    private ProgressListener<Result> progressListener;
    
    private LogicalModel logicalModel;
    private List<Implicant> mddImplicants;
    private List<Implicant> primeImplicants;
    private List<MaximalSymbolicSteadyState> maximalSymbolicSteadyStates;
    
    
    public Searcher(RegulatoryGraph regulatoryGraph, ProgressListener<Result> progressListener) {
	this.regulatoryGraph = regulatoryGraph;
	this.progressListener = progressListener;
    }
    
    public Result search() throws Exception {
	this.logicalModel = this.regulatoryGraph.getModel();
	// TODO: use Txt.t
	this.progressListener.milestone("Enumerate MDD implicants");
	enumerateMDDImplicants();
	// TODO: use Txt.t
	this.progressListener.milestone("Enumerate prime implicants");
	enumeratePrimeImplicants();
	// TODO: use Txt.t
	this.progressListener.milestone("Solve ILP");
	solveILP();
	Result result = new Result(primeImplicants, maximalSymbolicSteadyStates);
	return result;
    }
	...
	try {
	    ...
	    this.progressListener.setResult(result);
	} catch (Exception exception) {
	    this.progressListener.setResult(null);
	}
	return;
    }
